...
PY-K30|stage=5|scope=derived-surface-parity|kernel=when Python exposes one unit through derived sync async or threaded surfaces preserve the base unit's attached execution semantics so those surfaces do not silently change context propagation or concurrency behavior
...
PY-X30|stage=5|scope=derived-surface-drift|violation=do not expose derived sync async or threaded surfaces that silently change the base unit's context propagation or concurrency behavior
